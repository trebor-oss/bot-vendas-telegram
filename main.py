    ...
    while attempts < max_attempts:
        try:
            ...
            if search_result["response"]["results"]:
                ultimo_pagamento = search_result["response"]["results"][-1]
                ...
                # --- CENÁRIO 2: MENSAGEM VIP ---
                    ...
                    try:
                        await context.bot.edit_message_text(
                            chat_id=chat_id, 
                            message_id=message_id, 
                            text=f"✅ **CHAT VIP LIBERADO!**\n\n{nome_cli}, pode escrever sua mensagem abaixo. 👇", 
                            parse_mode='Markdown'
                        )
                    except: pass
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await query.edit_message_text(f"🔄 Gerando Link Seguro para {desc}...")
...
async def receber_mensagem_privada(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if verificar_permissao_msg(user_id):
        ...
        try:
            await context.bot.send_message(ADMIN_ID, f"💌 **MSG PAGA:**\n👤 {nome} (`{user_id}`)\n💬 {msg_cliente}")
            if update.message.photo or update.message.video or update.message.document: await update.message.forward(ADMIN_ID)
            await update.message.reply_text("✅ Recebido! Obrigada!")
...
async def broadcast(update, context, filtro):
    ...
    db = carregar_db()
    alvos = [uid for uid, d in db.items() if filtro == "todos" or (filtro == "pendentes" and d["status"] == "pendente") or (filtro == "compradores" and d["status"] == "comprador")]
    await update.message.reply_text(f"🚀 Enviando para {len(alvos)}...")
    for uid in alvos:
        try:
